...
```text
皮小闹冒失撞人(lines;旁白「咚!」标 mood:scary)
      ↓
   3 选项
...
| `t3-shape` | 形状层 | 缺省 | 2 题 → `onDone` 2 行 | 认出「钥匙」→ 到手 → 旁白「门开了」 |
| `t4-sound` | 声音层 | 缺省 | intro 整屏 → 2 题 | 进屋闷得慌,要那个「透气的口子」 |
| `t4-shape` | 形状层 | 缺省 | 2 题 → `onDone` 2 行 | 叫回「窗户」→ 风进来;皮小闹补一句「亮堂了」 |
| `t5-sound` | 声音层 | `atmosphere:'night'` | intro 整屏 → 2 题 | 天黑了;叫回「台灯」 |
| `t5-shape` | 形状层 | `atmosphere:'night'` | 2 题 → `onDone` 2 行 | 灯亮 → 「小小的屋子,一下子暖了起来」= **这一天成了** |
...
| `settle` | 结算 + 悬念 | 缺省(`day`) | `SettleCard` | 苏灵灵总括 → 徐万年一句「天天记得的事」也忘了 → **轻线痕迹**,不解释(5 行) |

- **`settle` 是唯一带「钩子」职责的幕**:每新增一句「还有下次」必须同时登记回收位(bible §6)。ch1 已埋项 = 徐万年「这个我天天都记得的呀」。
