...
- [ ] `P1` 笑点机制白名单裁定与扩容 — 2026-09-11 技能评审(`game-story-design` §十二 笑点配方)发现上行判据有三笔账:①**ch1 自身破格** —— `2026-09-11-qianzigu-ch1-rebuild-design.md` §6.3 把笑点 3「皮小闹撞人」登记为「**夸张**」,不在上行白名单 {误会·重复·落差} 内;②**上行 ch1 追溯登记与 spec 不一致** —— 上行记 2 处(推门拉反 / 皮小闹撞人),spec §6.3 记 3 处(+ `open` 徐万年卡壳);③**白名单撑不住连载** —— 仅 3 个机制,而 ch1 已吃掉「重复」(卡壳)与「落差」(拉反),ch2 只剩「误会」;全篇 20 章「章章有笑点」将在第三、四章必然重放。处置二选一:**扩白名单**至技能 §十二 机制库(落差 / 重复变奏 / 误会 / 错位 / 夸张 / callback / 角色驱动,仍禁谐音与双关)或**保留窄白名单 + 逐章轮换登记**。**阻塞**:ch1 重构 spec 定稿(笑点位冻结)与 ch2 立项均依赖本裁定。落点:`game-story-bible.md` §4 章节契约 + ch1 spec §6.3 + 本文件上行判据。2026-09-11 入池,未立项
