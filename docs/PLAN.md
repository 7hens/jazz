...
- [ ] `P0` `[feature]` 千字谷 ch1 重构(高潮 + 笑点 + 句型步切片 + 干扰项场景化)— 2026-09-11 产品口径「重构第一章,要有高潮和笑点」:①**章章有高潮有笑点** —— 消费想法池「章节叙事闸门」行,本章交付 = 章节契约补「高潮位 / 笑点位」两栏 + 剧情重写(现 ch1 笑点已在「推门拉反 / 皮小闹撞人」,高潮偏弱需重排);②**句型步先行切片** —— ch1 五词各配**多句、由易到难**,「声 → 形 → 句」三步连排(全量 100 词留想法池 P1);③选择题干扰项取**本章场景词 / 已学复习词**(替同 category 随机词,`question-engine` 接收场景词池入参)。⚠ 撞数据模型(word progress `sentence` 键 + 迁移)/ 幕数契约(18 幕变、断点重算)/ 出题引擎 —— **须先过 game-pipeline 阶段 0 重锚 + spec,勿直接开工**。— 阶段 0 已重锚,spec 已出待评审 — [design](superpowers/specs/2026-09-11-qianzigu-ch1-rebuild-design.md)
...
- [ ] `P1` 汉语句型步全量(每词多句 · 由易到难 · 拼装)— 原则「不止认字,还要帮孩子说话」:汉语域每词加第 3 步「声 → 形 → 句」,题型 = 句子模板挖空 + 4 选项点选成句(复用 `question-engine` 与可注入 `rng`);**不出声、不判音准** —— 守住「坚决不做:语音识别」红线。**每词多句、难度阶梯**(句长 / 结构由易到难),100 词各若干句**手写**(词库无词类,无法自动生成),句子存 `src/features/vocabulary/sentences.ts`(`wordId → sentence[]`,不塞 `words.ts`);选项干扰优先取**本章场景词 / 已学复习词**(见下行)。word progress 加 `sentence` 键 + 迁移;幕数变化、断点重算。⚠ **须与 0.3.0「汉语领域并轨 S1」的 `0004_chinese_domain.sql` 合并设计**(句型归汉语域内固定步,不新增第 3 开关),勿两条迁移并行。不做整句乱序拼装(数据 ×3,留二期)。**ch1 五词先行切片已升 0.3.0 feature 轨**(见上行),本行留**全量 100 词**。工作量 L → 强制拆 spec。2026-09-11 入池,同日修订为多句阶梯,未立项
