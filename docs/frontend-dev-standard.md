...
- 接口:`shared/services/<name>.ts`,**一文件一接口**;接口与同名 token const **一体**(token = `Symbol('<Name>') as ServiceToken<XService>`;`ServiceToken` 幽灵品牌类型与 `registry`/`useService`/`useServiceSnapshot` 注册·取用·订阅机制收 `services/manager.ts` 一文件):接口占 type 空间、token 占 value 空间,`services/index.ts` 统一出口,`import { XService }` 一个 import 同时拿类型与注册/取用 key。无集中 keys/map 映射。
