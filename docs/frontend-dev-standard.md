...
> 适用范围:本仓库前端 `src/`。**最终强制 = `src/architecture.test.ts`**(3 层边界 + useService 位置 + 注册纪律,违反即红);事实清单 = `CLAUDE.md`「架构」节;设计沿革与取舍 = `docs/superpowers/specs/2026-09-04-dev-architecture-refactor-design.md`。本文只记操作规则,不重复实现,冲突以测试与源码为准。原 `docs/ideas/2026-09-04-dev-architecture.md` 详案已删(不再用 `docs/ideas/`)。
...
- **凡在 `src/shared/**` 增删文件 ⇒ 必须同时 grep 并更新三处目录枚举**:本文件本节上一条、`CLAUDE.md`「架构」节的 `src/shared/` 行、`docs/dev-reference.md`「前端 3 层明细」的 `src/shared/` 行。只改其中一处 = 另外两处**从改的那一刻起就是假话**(2026-09-23 本支加 `ui/reward-card.ts` 与 `testing/` 时漏了这条,终审扫出)。命令(**命中那三处枚举 + 本规则自身** —— grep 的是上面那串字面,这条规则里也写着它,所以它一定会把自己数进去;**`grep` 出几行不在这里抄死,自己跑**):`grep -rn 'button / card / input / label' CLAUDE.md docs/`。
...
| 数据+逻辑 | 自含数据/规则;被跨 feature 消费时配无状态透传服务 | 经 shared 契约服务 | 可无 |
| 纯 UI | props 接收一切 | — | 无 |
...
- app 级跨 feature 组装提取为 `app/use*.ts` hook(如 `useAppState`);`App.tsx` 只表页面状态 + 组合关系,保持精简。
...
- **数据类随契约归属**:每个数据类由管理它的服务契约文件定义并持有(`PinyinProgressData`/`LevelStars`/`LevelClear`→`pinyin-progress.ts`、`UserSettings`→`settings.ts`、`Achievement`/`AchievementState`→`achievements.ts`、`ComboSnapshot`→`combo.ts`、`ApiError`/`User`/`Api*`→`api.ts`、`AudioCue`→`audio.ts`、`SpeechRole`→`speech.ts`),consumer 经 `services/index.ts` barrel 引入;无集中 `types.ts`。跨契约的 type-only 引用经 barrel 或直接相对路径(仅 shared 内部),随 `verbatimModuleSyntax` 一律 `import type`,type-only 环构建期擦除。**领域数据例外**:关卡/积木数据类(`Unit`/`Level`/`Syllable`/`Block`)不入 shared 契约,归 `features/pinyin-blocks` 并经其 `index.ts` 流出。
...
| 数据 / 全局常量 | UPPER_SNAKE | `UNITS` / `ACHIEVEMENTS` |
| 测试 | `<name>.test.ts` / `.test.tsx`,与被测同目录 | `rules.test.ts` |
...
- 位置与源文件同目录。
- 覆盖:关卡数据完整性 / 纯逻辑直调断言;服务(注入 fake,含加载 / 失败 / 乐观回滚);Entry 组装;`architecture.test.ts` 自动扫 3 层边界、useService 位置与注册纪律。
...
- feature 零跨引用;改关卡只动 `features/pinyin-blocks/levels.ts`,改玩法/结算规则先看 `features/pinyin-blocks/rules.ts` / `settle.ts` 与其测试,跨 feature 侧只经服务。
