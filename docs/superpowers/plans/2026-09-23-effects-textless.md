...
### 2. `W-C9` 这一行的前提列(期望列 / 操作列无偏离)

计划正文给出的 `W-C9`,**期望列与操作列**与交付文件里那一行**逐字一致**。**前提列有偏离**:按 `docs/walkthrough.md` §A 第 2 条补了 settings 就绪前提与「未验」出口 —— 交付的前提列是「任意设备,先登录(**settings 已就绪** ⇒ 顶部徽章栏已渲染出来)。**整条徽章栏没出现 ⇒ 此行记「未验」**(§D 第 4 条),**不许记「通过」**」(计划正文只写到「任意设备,先登录」)。

**为什么**:`src/features/pinyin-blocks/UnitMap.tsx` 的徽章栏是 `{earned === null ? null : ( … )}` —— settings 未就绪时**整条徽章栏不渲染**,那时期望列末句「徽章栏是一排图标,没有字」判不了;而原前提列没有「未验」出口 ⇒ 这一行会**静默算通过**。同表的 `W-V3` / `W-V4` / `W-V5` 都有这个出口,`W-C9` 独缺。判定桶是**逐条目**的(通过 / 未通过 / 未验),故整行记「未验」,不发明第四种状态。
...
**实际结果**:交付文件的口径是**会话连击** —— 存 `sessionStorage` 的 `mgp_combo`、**跨关累计**、放错一块即清零、封顶 5 颗;前提 / 操作列按会话级重写(含「开始前先故意放错一块把连击归零」「u1 每关只有 2 个槽,要跨关才连得起来」)。**期望列也变了**(计划正文里没有这两件事):计划正文的期望是「右上角圆点**逐颗点亮**;**第 6 块故意放错 → 全部归零**(暗回去)」,交付的期望是「圆点**逐颗点亮**(**第 n 块亮 n 颗**);**最多亮 5 颗**(第 6 块不再多亮);放错一块 → 全部归零(暗回去)」—— 多出「第 n 块亮 n 颗」这个逐颗口径与「最多亮 5 颗」这个封顶。**封顶 5 颗是 `combo` 的实现口径**,出处 `src/features/combo/`(`LevelEntry.tsx` 的连击圆点注释同样写「5 颗封顶」)。
...
- **核对清单**(计划 Step 3 的 grep):原清单漏 `W-V4` / `W-P5` / `W-P6`,已补齐后再核(见第 8 条)。
- **引用清单**(计划 Step 4 的「见 …」):原清单漏 `W-V4` / `W-P5` / `W-V5`;且**「成就的可见反馈」在孩子面那一半原本没有任何一行**,故先新增 `W-V5` 再引用(见第 6 条)。
- **`W-C5` 注的落点**(计划 Step 5):原文写「在 §B 的 `W-C5` 那行**之后**加一段注」—— 表格中间插 `>` 块会把表切成两张,故注落在 §B 表末、与已有的 `W-C3 / W-C4` 的注同一处(同一种写法);注的正文逐字未改。
- **`W-V1` 与 §E 的「徽章行」**:与本支新造的「徽章栏」(顶部成就图标条)只差一字、且同表出现,已按 rider 改成「格子里那排积木名片」;**`W-V1` / `W-V2` 前提列缺状态前提是分支前就有的缺陷,不在本支射程,未动**(要修先进 `docs/PLAN.md` 想法池)。
- **`W-P6` 前提的「同上」**:会链式继承紧邻上方 `W-P5` 前提里的「且先确保**至少已得一条**」,与 `W-P6` 判「**未得**行的说明读得清」相斥(全新账号一律记「未验」,恰在最该判的时候不判);已改成「同 `W-P4` 前提」—— **只改这一个词组**。
- **`docs/PLAN.md` 那一行**(计划 Step 6):原文只要求改勾选框与行尾;实际中段的「诊断」是**立项时口径**,而本支已把其中五处变成假话,故把它标定为立项时口径,并在行尾补「实施后实况」。行内「**进 0.2.0**:0.2.0 未发、未 push」一处**今天仍为真,未动**。
...
**为什么**:本节开头写的是「本节记录本支执行结果与计划正文的偏离」,所以这句过期枚举也在记录之内。其中 **`W-V5` 是必须的** —— 缺它,`docs/walkthrough.md` §C 那句「已纳入」在它自己列的清单上为假(交付的 `W-V4` 只判**未得**格看得见,不判**拿到之后**那一格亮起;见第 6 条)。
