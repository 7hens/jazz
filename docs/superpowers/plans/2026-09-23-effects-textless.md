...
**为什么**(前提列):`src/features/pinyin-blocks/UnitMap.tsx` 的徽章栏是 `{earned === null ? null : ( … )}` —— settings 未就绪时**整条徽章栏不渲染**,那时期望列末句判不了;而原前提列没有「未验」出口 ⇒ 这一行会**静默算通过**。同文件另三条徽章栏行(`W-V3` / `W-V4` / `W-V5`,都在 `§C`「布局与窄屏」表)都有这个出口,`W-C9` 独缺。判定桶是**逐条目**的(通过 / 未通过 / 未验),故整行记「未验」,不发明第四种状态。

**为什么**(期望列末句):发布闸门①(§B 核心回归)用的是 preview 独立 D1 + **全新库**;全新账号上**一个成就都没拿到**,徽章栏按目录全量渲染但**未得的格子不画图标** ⇒ 整排空圈、零图标,「是一排图标」在**该闸门最常跑的环境里为假**。改成「一排**格子**(没拿到的那几格是空圈)」后,全新账号与已得若干成就的账号两种状态下都为真;断言仍是**没有字**这一件事(新加的是描述,不是第二件判据)。

**为什么**:`src/features/pinyin-blocks/UnitMap.tsx` 的徽章栏是 `{earned === null ? null : ( … )}` —— settings 未就绪时**整条徽章栏不渲染**,那时期望列末句(判徽章栏那半句)判不了;而原前提列没有「未验」出口 ⇒ 这一行会**静默算通过**。同文件另三条徽章栏行(`W-V3` / `W-V4` / `W-V5`,都在 `§C`「布局与窄屏」表;`W-C9` 在 `§B`)都有这个出口,`W-C9` 独缺。判定桶是**逐条目**的(通过 / 未通过 / 未验),故整行记「未验」,不发明第四种状态。
...
**实际结果**:交付文件的口径是**会话连击** —— 存 `sessionStorage` 的 `mgp_combo`、**跨关累计**、放错一块即清零、封顶 5 颗;前提 / 操作列按会话级重写(含「开始前先故意放错一块把连击归零」「u1 每关只有 2 个槽,要跨关才连得起来」)。**期望列也变了**(计划正文里没有这两件事):计划正文的期望是「右上角圆点**逐颗点亮**;**第 6 块故意放错 → 全部归零**(暗回去)」,交付的期望(逐字引 `docs/walkthrough.md` 那一行的期望列)是「右上角圆点**逐颗点亮**(第 n 块亮 n 颗);**最多亮 5 颗**(第 6 块不再多亮);**放错一块 → 全部归零**(暗回去)」—— 多出「第 n 块亮 n 颗」这个逐颗口径与「最多亮 5 颗」这个封顶。**封顶 5 颗是消费侧(关卡页连击圆点)的口径**,出处 `src/features/pinyin-blocks/LevelEntry.tsx` —— 该文件的 `const COMBO_DOTS = [0, 1, 2, 3, 4]`(5 项)配 `const comboLit = Math.min(comboSnap.combo, COMBO_DOTS.length)`,把连击值夹到这张表;`src/features/combo/combo.ts` 的 `nextCombo` 本身**无上限**(那里唯一的 `Math.min(combo * 5, 50)` 属于 `comboBonus`,封的是**星尘加成**的 50,不是格数)。
...
**实际结果**:三列都重写。起点按会话级连击界定;期望新含「**通关那一块另出一记撒花**」(另一档,与连击档先后相继、不同帧)与两条让位规则(弹成就弹层 ⇒ 该记撒花改由成就那一档来撒;弹幸运弹层 ⇒ 这一关的通关撒花当场不出,连击档不受影响)。

**为什么**:交付实现的撒花有四档(出处 `src/features/celebrate/celebrate.ts`),计划文本只覆盖连击两档,且没有「弹层档出现时通关档让位」这条 —— 留着会把实现记成缺陷。
...
**实际结果**:交付的 `W-V3` 标题带「(布局)」、前提补「已登录并停在地图页(`settings` 已就绪 ⇒ 徽章栏已渲染出来)」与「整条徽章栏没出现 ⇒ 记「未验」」这条退化出口、期望首加「**一行放得下**:」;并把「未得的那几格看得见是一个空位」从 `W-V3` **拆出来另立 `W-V4`**;本支收尾又新增 **`W-V5`**(拿到成就后徽章栏那一格亮起来)。计划正文里**没有** `W-V4` / `W-V5` 这两行。
...
- **`W-P6` 前提的「同上」**:会链式继承紧邻上方 `W-P5` 前提里的「且先确保**至少已得一条**」,与 `W-P6` 判「**未得**行的说明读得清」相斥(全新账号一律记「未验」,恰在最该判的时候不判);已改成「同 `W-P4` 前提」—— **只改这一个词组**。
- **`docs/PLAN.md` 那一行**(计划 Step 6):原文只要求改勾选框与行尾;实际中段的「诊断」是**立项时口径**,而本支已把块内多处**现在时态的状态描述**变成假话,故把它标定为立项时口径,并在行尾补「实施后实况」。**这里不写死「几处」** —— 处数是个派生数,随块内任何一次改动失真(本支已反复为「写死乘积 / 写死条数」付费);按规则写:凡**现在时态的具体状态描述**都是实施前实况,以行尾「实施后实况」为准,而**四条裁定本身 / 不在射程 / 与想法池的关系**今天仍成立(裁定是决定,不是状态)。行内「**进 0.2.0**:0.2.0 未发、未 push」一处**今天仍为真,未动**。
