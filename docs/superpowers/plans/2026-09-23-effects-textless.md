...
**归属更正(2026-09-24 终审 rider 补记 —— 下面的记述内容一字未改,只更正它今天挂在哪个名字下)**:计划正文这一行在交付文件里**拆成了两行**,本条原先一并记在 `W-L5` 名下的两类内容,今天挂在 **`W-L6`** 下:
...
- **拆分出处(可查)**:`git log -S "W-L6" -- docs/walkthrough.md` → `b8577b1`;该提交把这次拆分记在自己的 **B 组「走查行粒度」**下,原文即「`W-L5` 拆为 `W-L5`(连击档)与 `W-L6`(通关档与让位规则)」。
- **为什么这么切**:两类的**触发时机不同** —— 这句是交付 `W-L6` 行期望列自己写的(「连击档随落块当场出,通关档要等成功动画走完」,`grep -n "W-L6" docs/walkthrough.md` 可见);按 `§A` 第 4 条「一条只判一件事,两件事写两条」,判据不同即分行。
