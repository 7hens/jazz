...
## 执行期更正(2026-09-23,执行后追加)

> 本节记录**本支执行结果与上方计划正文的偏离**。计划正文**原始保留、不追改** —— 它是派发时的指令记录;以本节为「实际发生了什么」的入口。
> 定位一律**引原文**(不引行号:行号会随任何一次编辑失效)。

**核对动作(可重跑)**:计划正文里的走查行引文只有 `W-S3` / `W-L4` / `W-L5` / `W-V3` / `W-P4` / `W-C9` 这几个名字 —— 由 `grep -n "^| \*\*W-" docs/superpowers/plans/2026-09-23-effects-textless.md` 得出(**注意:本节下面的「计划正文原文」引用块里也有 `| **W-` 开头的行,那几条 grep 会一并命中**);逐一与 `grep -n "^| \*\*W-" docs/walkthrough.md` 里同名行的当前文本对照。**两边各有多少条不在这里抄死** —— 要数请自己跑这两条 grep。
...
```
**为什么 `W-C9` 留到最后(裁定):** 它的第一句前提是「地图 + 关卡 + 弹层三面都已改完」。提前写就是对当时为假的事实写期望 —— 本仓病根之一。
```

**实际结果**:计划正文紧接着给出的 `W-C9` 行文,前提列是「任意设备,先登录」—— 那句「第一句前提是三面已改完」**对文本为假**。执行时删掉这条假描述(裁定「留到最后」本身保留),并补上「前提列为什么不写施工状态」的理由。

**为什么**:病根就是「文档承诺一个不存在的事实」。另外「三面已改完」是**作者**的施工状态,不是走查者能判定、做不到就能记「未验」的前提(`docs/walkthrough.md` §A 第 2 条要的是设备 / 宽度 / 刷新这类)。

### 2. `W-C9` 这一行逐列对账(改了哪几列、分别为什么)

**实际结果**(**不写「整行逐字一致」这类全称断言** —— 计划正文与交付文件两边都不冻结,那种断言会被下一次改动推翻;逐列对账,对的是**本轮交付时的文本**):

- **操作列**:与计划正文**逐字一致**(本支未动)。
- **前提列**:**改过**。按 `docs/walkthrough.md` §A 第 2 条补了 settings 就绪前提与「未验」出口 —— 交付的前提列是「任意设备,先登录(**settings 已就绪** ⇒ 顶部徽章栏已渲染出来)。**整条徽章栏没出现 ⇒ 此行记「未验」**(§D 第 4 条),**不许记「通过」**」(计划正文只写到「任意设备,先登录」)。
- **期望列**:**改过**(改的是**末句**,其余部分与计划正文一致)。末句从「徽章栏是一排图标,没有字」改成「徽章栏是一排**格子**(没拿到的那几格是空圈),没有字」。

**为什么**(前提列):`src/features/pinyin-blocks/UnitMap.tsx` 的徽章栏是 `{earned === null ? null : ( … )}` —— settings 未就绪时**整条徽章栏不渲染**,那时期望列末句(判徽章栏那半句)判不了;而原前提列没有「未验」出口 ⇒ 这一行会**静默算通过**。同文件另三条徽章栏行(`W-V3` / `W-V4` / `W-V5`,都在 `§C`「布局与窄屏」表;`W-C9` 在 `§B`)都有这个出口,`W-C9` 独缺。判定桶是**逐条目**的(通过 / 未通过 / 未验),故整行记「未验」,不发明第四种状态。

**为什么**(期望列末句):**一个成就都没拿到时,徽章栏整排是空圈、零图标**(徽章栏按目录全量渲染,**未得的格子不画图标** —— 见 `src/features/pinyin-blocks/UnitMap.tsx` 里 `got` 为假时不渲染 `badge.emoji` 那个分支) ⇒ 旧末句「是一排图标」在**该状态下**为假。改成「一排**格子**(没拿到的那几格是空圈)」后,**该状态**与「已得若干成就」的状态下都为真;断言仍是**没有字**这一件事(新加的是描述,不是第二件判据)。**本句不断言走查跑在哪种库上、也不断言哪种状态更常出现** —— 那是环境事实,本记录没有依据。
...
**计划正文原文**(Task 2 Step 7 的最终态)

```
| **W-S3** 奖励弹层 | 任意设备 | 拿到一次成就;再拿到一次幸运奖励 | 弹层里**没有文字**,只有图标 + `+N ⭐`;**两个弹层卡片高度一致**(切换时不跳) |
```

**实际结果**:交付文件的期望列与最终态一致;操作列在「再拿到一次幸运奖励」之后补了随机性与退化出口:「**注意幸运弹层是随机的**:只在**首通一关**时掷,**概率不高**(常量 `LUCKY_RATE`,出处 `src/features/lucky-bonus/lucky-bonus.ts` —— 要核对具体数值请去那里看,本行不抄死它)。**一次没出弹层不等于缺陷**,再多首通几关直到出为止」。

**为什么**:幸运弹层是掷骰子出的 —— 不写退化出口,走查者会把「一次没出」当缺陷;数值只指出处文件、不抄死(数值会变,抄进走查就会静默变假)。
...
**实际结果**:交付文件的口径是**会话连击** —— 存 `sessionStorage` 的 `mgp_combo`、**跨关累计**、放错一块即清零、封顶 5 颗;前提 / 操作列按会话级重写(含「开始前先**故意放错一块**把连击归零」「u1 每关只有 2 个槽,所以**要跨关才连得起来**」)。**期望列也变了**(计划正文里没有这两件事):计划正文的期望是「右上角圆点**逐颗点亮**;**第 6 块故意放错 → 全部归零**(暗回去)」,交付的期望(逐字引 `docs/walkthrough.md` 那一行的期望列)是「右上角圆点**逐颗点亮**(第 n 块亮 n 颗);**最多亮 5 颗**(第 6 块不再多亮);**放错一块 → 全部归零**(暗回去)」—— 多出「第 n 块亮 n 颗」这个逐颗口径与「最多亮 5 颗」这个封顶。**封顶 5 颗是消费侧(关卡页连击圆点)的口径**,出处 `src/features/pinyin-blocks/LevelEntry.tsx` —— 该文件的 `const COMBO_DOTS = [0, 1, 2, 3, 4] as const`(5 项)配 `const comboLit = Math.min(comboSnap.combo, COMBO_DOTS.length)`,把连击值夹到这张表;`src/features/combo/combo.ts` 的 `nextCombo` 本身**无上限**(那里唯一的 `Math.min(combo * 5, 50)` 属于 `comboBonus`,封的是**星尘加成**的 50,不是格数)。

**为什么**:实现是跨关累计的会话计数,不是「本关连对 5 块」—— 按计划文本走查必红(单关根本连不到 5 块)。

### 5. `W-L5`(交付拆成两行:连击档 `W-L5` + 通关档与让位规则 `W-L6`)
...
**实际结果**:三列都重写。起点按会话级连击界定。

**归属更正(2026-09-24 终审 rider 补记)**:计划正文这一行在交付文件里**拆成了两行**,本条原先一并记在 `W-L5` 名下的两类内容,今天挂在 **`W-L6`** 下。**本条正文不是初写原文** —— 上面那句在本补记里被拆开、又接了一串 bullet,连接词与粗体都动过(逐字的旧版本见 `git show 6498bfd:docs/superpowers/plans/2026-09-23-effects-textless.md`,本支不另存):

- **`W-L5`**(交付标题「撒花档位(连击档)」)承接计划正文原有那三件:第 5 块一记小撒花 / 第 10 块一记大撒花 / 其余单块放对不出撒花。
- **`W-L6`**(交付标题「撒花档位(通关档与让位规则)」;**计划正文里没有这一行**)承接另外两类:(a) **通关那一块另出一记撒花**(另一档,与连击档先后相继、不同帧);(b) **两条让位规则**(弹成就弹层 ⇒ 该记撒花改由成就那一档来撒;弹幸运弹层 ⇒ 这一关的通关撒花当场不出,连击档不受影响)。
- **拆分出处(可查)**:`git log -S "W-L6" -- docs/walkthrough.md` → `b8577b1`;该提交把这次拆分记在自己的 **B 组（走查行粒度）** 下(原文在该提交信息里自看,本节不转抄)。
- **为什么这么切**:两类的**触发时机不同** —— 这条差别交付的 `W-L6` 行自己写着(`grep -n "W-L6" docs/walkthrough.md` 可见);按 `docs/walkthrough.md` 的 `§A` 第 4 条(规则原文在该文件里,本节不转抄),判据不同即分行。
- **可重跑核对**:`grep -n "W-L6" docs/walkthrough.md`(该名字在交付走查文件里的落点;要看这几件事今天各自归谁,以本节的归属为准)。

**为什么**(对交付的这两行一起看):交付实现的撒花有四档(出处 `src/features/celebrate/celebrate.ts`),计划文本只覆盖连击两档,且没有「弹层档出现时通关档让位」这条 —— 留着会把实现记成缺陷。

### 6. `W-V3`(拆出 `W-V4`;交付另有 `W-V5`)
...
**实际结果**:交付的 `W-V3` 标题带「(布局)」、前提补「已登录并停在地图页(**settings 已就绪** ⇒ 徽章栏已渲染出来)」与「**整条徽章栏没出现 ⇒ 此行记「未验」**」这条退化出口、期望首加「**一行放得下**:」;并把「未得的那几格看得见是一个空位」从 `W-V3` **拆出来另立 `W-V4`**;本支收尾又新增 **`W-V5`**(拿到成就后徽章栏那一格亮起来)。计划正文里**没有** `W-V4` / `W-V5` 这两行。

**为什么**:`§A` 第 4 条一条只判一件事 —— 「布局放得下」与「空圈看得见」是两件事。`W-V5` 补的是**成就的可见反馈**在孩子面唯一那条人眼出口;缺它,`docs/walkthrough.md` §C 那句「已纳入」在它自己列的清单上为假(交付的 `W-V4` 只判**未得**格看得见,不判**拿到之后**那一格亮起)。
...
**实际结果**:交付拆成三行 —— `W-P4`(目录全量 + 每条三要素)/ `W-P5`(已得 / 未得两态可区分)/ `W-P6`(未得行的说明读得清);`W-P4` 的条数改成「以 `src/features/achievements/achievements.ts` 为准 —— 要核对具体条数请去那里看,本行不抄死它」。

**为什么**:写死「**8 条全在**」就是今天对、明天假的数(目录一变即假);三件事(全量 / 两态 / 可读性)拆三行是按 `§A` 第 4 条。两态的承重判据也从「压暗显示」收窄到两处(「· 已得」+ 图标浓淡)—— 虚线描边与底色经算过低于阈值,**不作判据**。
...
```
**本任务只收**跨面 / 收口的那几条:每个改动面的分区条目(`W-S3` / `W-L4` / `W-L5` / `W-V3` / `W-P4`)已经跟着**各自的行为提交**落进 `docs/walkthrough.md` 了
```

**实际结果**:本支实际落进交付文件的分区条目还包括 **`W-V4`** / **`W-P5`** / **`W-P6`** / **`W-V5`**。执行时的核对 grep 已按 `W-S3|W-L4|W-L5|W-V3|W-V4|W-P4|W-P5|W-P6` 跑过。

**为什么**:核对清单漏项比没有清单更糟 —— 它会产出「清单上那几条都在 ✓」的绿色结论,而漏掉的恰是缺陷面。

### 9. `8 × (24 + 6) = 240px` 那段算好的值

**计划正文原文**(UnitMap 插入徽章栏的代码块注释)

```
独立一行 + flex-wrap,不吃格子内宽:8 × (24 + 6) = 240px < 375 − 32 ✓(UnitMap.test 钉输入,人眼由 W-V3 兜)。
```

**实际结果**:交付代码里没有这串数,改成**写规则不写值**:「独立一行 + flex-wrap,不吃格子内宽:每格 `h-8 w-8` + `gap-1.5`,乘目录条数须落在窄屏预算内(人眼由 `W-V3` 兜)。」

**为什么**(两步):① 那串数本身是错的 —— `h-8` 是 **32px** 不是 24px(24px 是 `h-6`);按 32px 重算,整行宽 = **条数×32 +(条数−1)×6**(间隔比条数少一个)。成就目录**当前 8 条** ⇒ 8×32 + 7×6 = **298px**(不是 304 —— 那是给 8 个方块算了 8 个间隔),仍在 375 视口的 343px 预算内 ⇒ 结论仍成立。② 更根本的是「8 ×」这一半:成就目录条数一变,写死的乘积就**静默变假** —— 造好的数写进交付物,正是本支立项要灭的那类东西。

### 10. `reward-card.ts` 注释块里「高度一致由共用这一份保证」

**计划正文原文**(该文件的注释)

```
取的意图是「文字消失,卡片不变形」,不是精确复刻 —— 两个弹层高度一致由**共用这一份**保证,不由这个数值保证;数值本身好不好看由走查 `W-S3` 兜人眼。
```

**实际结果**:交付文件同一段改成「共用这一份保证的是两个弹层**几何约束相同**(`min-h` / `max-w` / `p-6` / 描边),**不保证渲染高度相同**(哪天某一边的内容高过 `min-h`,两边就会不等)」。

**为什么**:计划那句是**假的** —— 共用一份类名只保证下限 / 宽度 / 内距相同,内容高过 `min-h` 时两边就不等;复审后改成只承诺兑现得了的那半句。

### 11. 收尾那句把 `W-L4` / `W-L5` 与真机项写在一起
...
**实际结果**:交付文件里 `W-L4` / `W-L5` 的前提列都是「任意设备」,**没有**需要真触摸设备才能验的部分;本支的真机条目仍是 `W-C3` / `W-C4`。此句为前提式表述(「需要…的部分」),在交付上**无对应项**。

**为什么**:记一笔,免得走查者去找一个不存在的「W-L4/W-L5 真机部分」;走查一律以 `docs/walkthrough.md` 为准。

### 12. Task 8 本体的执行相对计划正文的偏离(控制器 rider 裁定,2026-09-23)

- **提交信息**(计划 Step 8 给出的 `git commit -m`):原文声称本提交「§C 加 W-L4/W-L5/W-S3/W-V3/W-P4」—— 那五行是**各自的行为提交**落的,本提交只是**核对**;原文也漏了本提交实际做的改动。已按**实际动作**改写提交信息。
- **核对清单**(计划 Step 3 的 grep):原清单漏 `W-V4` / `W-P5` / `W-P6`,已补齐后再核(见「计划正文的走查行引用清单漏项」那条)。
- **引用清单**(计划 Step 4 的「见 …」):原清单漏 `W-V4` / `W-P5` / `W-V5`;且**「成就的可见反馈」在孩子面那一半原本没有任何一行**,故先新增 `W-V5` 再引用(见「`W-V3`(拆出 `W-V4`;交付另有 `W-V5`)」那条)。
- **`W-C5` 注的落点**(计划 Step 5):原文写「在 §B 的 `W-C5` 那行**之后**加一段注」—— 表格中间插 `>` 块会把表切成两张,故注落在 §B 表末、与已有的 `W-C3 / W-C4` 的注同一处(同一种写法);注的正文逐字未改。
- **`W-V1` 与 §E 的「徽章行」**:与本支新造的「徽章栏」(顶部成就图标条)只差一字、且与本支新增的徽章栏行(`W-V3` / `W-V4` / `W-V5`)同处 `§C`「布局与窄屏」表,已按 rider 改成「格子里那排积木名片」;**`W-V1` / `W-V2` 前提列缺状态前提是分支前就有的缺陷,不在本支射程,未动**(要修先进 `docs/PLAN.md` 想法池)。
- **`W-P6` 前提的「同上」**:会链式继承紧邻上方 `W-P5` 前提里的「**且先确保至少已得一条**」,与 `W-P6` 判「未得的说明读得清」相斥(全新账号一律记「未验」,恰在最该判的时候不判);已改成「同 `W-P4` 前提」—— **只改这一个词组**。
- **`docs/PLAN.md` 那一行**(计划 Step 6):原文只要求改勾选框与行尾;实际中段的「诊断」是**立项时口径**,而本支已把块内多处**现在时态的状态描述**变成假话,故把它标定为立项时口径,并在行尾补「实施后实况」。**这里不写死「几处」** —— 处数是个派生数,随块内任何一次改动失真(本支已反复为「写死乘积 / 写死条数」付费);按规则写:凡**现在时态的具体状态描述**都是实施前实况,以行尾「实施后实况」为准,而**裁定本身 / 不在射程 / 与想法池的关系**今天仍成立(裁定是决定,不是状态)。行内「**进 0.2.0**:0.2.0 未发、未 push」一处**今天仍为真,未动**。

### 13. 计划正文 Global Constraints 里那句「Task 8 只收…」的枚举已过期

**计划正文原文**

```
- **改用户可见行为 ⇒ 同一个提交里更新 `docs/walkthrough.md`**:…**不要**把分区条目攒到 Task 8 —— Task 8 只收跨面的 `W-C9`、条目计数、两条诚实缺口、`W-C5` 加注与 PLAN 收口。
```

**实际结果**:Task 8 实际还做了四项 —— 新增 `W-V5`、改 `W-P6` 的前提词组、两处「徽章行」改名、追加本计划文件这一节本身。

**为什么**:本节开头写的是「本节记录本支执行结果与计划正文的偏离」,所以这句过期枚举也在记录之内。其中 **`W-V5` 是必须的** —— 缺它,`docs/walkthrough.md` §C 那句「已纳入」在它自己列的清单上为假(交付的 `W-V4` 只判**未得**格看得见,不判**拿到之后**那一格亮起;见「`W-V3`(拆出 `W-V4`;交付另有 `W-V5`)」那条)。

### 14. 本支的副作用:Step 2 把 `§B` 从 8 条改成 9 条,连带让 `release/SKILL.md` 闸门①那句变假

**本支的副作用**:Step 2 把 `§B` 从 8 条改成 9 条,**连带**使 `.claude/skills/release/SKILL.md` 闸门①那句「§B 核心回归(**8 条**)」变成假话 —— 该文件本支原本未动,是这句话**被本支变成了假的**。已删去那个数(那句自己就写着「此处不再内嵌,两处写一份必漂移」)。

**为什么记这条**:它是一个「我改了 A,A 让没动过的 B 开始撒谎」的实例 —— 这类连带损伤是本支最贵的复发位(「修复即新面」),留一条记录,后来人才会去 grep 同类引用。**去数而不是更新数**:改成「9 条」只是把这次的漂往下推一次,下一支加一条 `W-C10`,同一句话会再假一次。

### 15. `celebrate.ts` 档位注释(计划正文 Task 4 Step 4 的代码块)

**计划正文原文**(Task 4 Step 4 的那个 `ts` 代码块;块前一行自标它对应 `src/features/celebrate/celebrate.ts:6-11`。块里其余行去那里看,下面只引今天为假的那一行)

```
// 一关 15 块 × 30 粒 = 撒 15 次,孩子很快就不看了;单块反馈已有槽位填色 + correct 音效(设计 §3.4)。
```

**实际结果**:交付文件 `src/features/celebrate/celebrate.ts` 顶部那条档位注释**在 `6498bfd` 里改过**(改后全文:`git show f09b395:src/features/celebrate/celebrate.ts` —— 取固定 sha,别用 `HEAD~N`)。改法:按源码实际播的 cue 命名落块反馈(`'tap'`,出处 `features/pinyin-blocks/PinyinBlocksGame.tsx` 的 `placeBlock`);写明 `AudioCue` 的 `'correct'` 今天**生产零调用点**;去掉示例数,改成不写死块数(落块次数 = 该关槽位数、托盘块数另算)。同一句的规格侧副本,勘误落在同一次提交的 `docs/superpowers/specs/2026-09-23-effects-textless-design.md` §3.4。

**为什么**:上面引的那一行今天为假 —— 它把一关的落块次数写成相乘的得数,而实际落块次数 = **该关槽位数**,不是那个乘积;它又把落块反馈说成 `correct` 音效(那一档生产里零调用点)。**乘积类数字正是本支立项要灭的东西**:底层增删后它静默变假。计划正文那个块按本节既定规矩**原始保留、不追改**,故这条偏离只记在这里;要看交付与规格今天各写什么,按上面列出的指针去读原文。
