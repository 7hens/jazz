...
计划正文给出的 `W-C9` 整行,与交付文件里那一行**逐字一致**。
...
**实际结果**:交付文件的口径是**会话连击** —— 存 `sessionStorage` 的 `mgp_combo`、**跨关累计**、放错一块即清零、封顶 5 颗;前提 / 操作列按会话级重写(含「开始前先故意放错一块把连击归零」「u1 每关只有 2 个槽,要跨关才连得起来」)。
...
**为什么**(两步):① 那串数本身是错的 —— `h-8` 是 **32px** 不是 24px(24px 是 `h-6`);按 32px 重算,每格 32 + 间距 6 = 38px,乘 8 = **304px**,不是 240px(预算 375 − 32 = 343px,304 < 343,结论仍成立)。② 更根本的是「8 ×」这一半:成就目录条数一变,写死的乘积就**静默变假** —— 造好的数写进交付物,正是本支立项要灭的那类东西。
