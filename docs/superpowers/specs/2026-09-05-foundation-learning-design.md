...
> 日期:2026-09-05 · 状态:设计定稿,待实现 → 已实现(0.2.0) · 所属:feature 轨(新能力,兼容 → minor,0.3.0 目标)
>
> ⚠️ **后续修订(并入 0.2.0,本 spec 以下正文为当时设计,与终态有出入,以 PLAN 行与源码为准)**:①「演示→点读→轻测」三步短教 → 0.2.0 起短教改**纯判分 4 选项题**(删 demo/tap 演示步,删听齐门控),见 `2026-09-06-quiz-interaction-redesign-design.md` 及其收口修订;②「软提示(soft)浮条」→ 已砍,soft 一律放行直接答题。§8 为原始三步设计,已被取代,看交互终态请读 dev-reference「题型与发音约定」与 quiz-interaction spec 修订注解。
>
...
2. 评估方式:**被动推断为主 + 冷启动短测**。零进度新儿童首次跑 5-8 题梯度短测;平时由短教轻测喂估计器,不扩 lesson 逐题埋点。
3. 教学出现方式:**首次强制,后续不打断(0.2.0 起)** —— 弱单元且从未教过 → 该词该技能步前**先学后答**;教过仍在 learning → 直接答题、不再弹软提示;known → 不出现。
...
- `soft` → 放行直接答题(0.2.0 发布前定稿砍掉原「想先学一下?」软浮条:教过仍 learning 不再每词每步打断;仅未教 `mandatory` 才插强制短教)。
...
- 短教强制步中途退出地图 → 不写熟度、不罚星、`taughtCount` 不变 → 下次仍强制。

## 8. 短教三步交互(`features/foundation/TeachOverlay`)

> ⚠️ **本节被取代(0.2.0 收口)**:终态短教 = 对每个目标 unit 直接出 **4 选项纯判分题**(复用 Choice/ListenChoice),不再有下述「演示拆合 → 点读跟读」两步;错答仍复演示该单元锚点 + 可重试,全过 praise 结课。见 dev-reference「题型与发音约定」与 PLAN 收口行。
