...
| D4 | 提交钮措辞 | **「就它了!」** —— 字面即 `就它了` + **ASCII `!`**(U+0021),非全角 `!`(U+FF01) | 去掉「交卷」signifier;两语境(孩子答题 / 家长教学)都读得通。**仅 `Choice`/`ListenChoice` 有提交钮**;`MatchGame` 配对即判、无此钮,不受 D4 影响。**全半角写死**是因为测试断言用精确字符串匹配,混用会静默失配 |
...
| 答错反馈 | 红框 + 抖动 | 红框 + **✗ 角标** + 保留抖动(**不加透明度压暗** —— 会破 §7 对比下限) | `red` / `red-tint` |
...
| 对比 | ≥ 4.5:1 | 气泡文字 `text-ink on bg-surface` ✓;accent-tint 上文字用 `accent-ink` ✓。**本轮新增动效一律不用 `opacity` 压暗** —— 压暗会破 4.5:1;既有 `MatchGame` 已配对卡的 `opacity-80` **保留**(静态态,文字仍在 `bg-emerald/10` 上,实施时目视复核) |
...
- `stone.ts` 四态函数:纯函数单测(默认/选中/对/错 的类名与角标字形)
- ✓/✗ 角标:**`aria-hidden="true"` + 按钮挂 `data-state`**,测试断言 `button[data-state="correct"]` 与角标存在(色彩冗余的可测化)
- 「就它了!」的 disabled→enabled 状态机:沿用 `Choice.test.tsx` 既有两条断言,仅改名字
- 教学期区分:断言 `TeachOverlay` 渲染「📖 演示」标签(可测的结构,非颜色)

> ⚠ **硬约束(实施前必读)**:测试**全部靠 accessible name 定位选项**(`getByRole('button', { name: '档1正确' })` / `{ name: '太阳' }`,见 `scene-ui.test.tsx` `ChapterRunnerView.test.tsx` `WordLesson.test.tsx` 等)。因此任何新增的角标/修饰节点**必须 `aria-hidden="true"`**,否则会改变选项按钮的 accessible name → 大面积测试崩。同理,提交钮的 ✨ **不得**用裸 emoji 文本节点(用 `<span aria-hidden>✨</span>`)。
