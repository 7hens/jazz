...
> ⚠ **2026-09-23 勘误(上句原文一字未改)**:本版**未实现**星级揭晓与「继续」按钮。实际行为 = 拼齐 → 答案行亮出(拼音 + 汉字)并朗读 → 约 260ms 判定 + 1600ms 停顿 → **自动**进下一关,**单元最后一关自动回地图**(`src/features/pinyin-blocks/LevelEntry.tsx:69-70`);关内**不显示星** —— 星只作为**地图格子上的点/灭两态**可见(`UnitMap.tsx` 的 `.pstar`),看不出某关是 1 / 2 / 3 星。是否补关卡内星级即时反馈,见 `docs/PLAN.md` 想法池。
