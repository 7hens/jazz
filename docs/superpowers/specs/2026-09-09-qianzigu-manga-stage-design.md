...
- ▶ 2026-09-09 补充批已消费其中「天空/布景实景化 + 太阳本体进度尺 + 世界回春」核心子集,见 **§15**(并建议 T6 走查并入该批后一并做)。
...
## 15. 补充批 · 舞台布景实景化 + 太阳复原(2026-09-09)

> 在 §5-§7 既有「氛围层 + 词 emoji 点灯条」之上,把天空从「氛围渐变 + 顶部词条」升级为**千字谷实景布景 + 太阳本体进度尺**。纯 `qianzigu` 呈现层 + `chapter.ts` 加**一个可选字段**;引擎/进度/语音/speech 契约/断点续玩**仍零改动**。
> 决策(用户拍板 2026-09-09):太阳**常驻天空为天幕主位**,当幕由 scene 数据决定「天空体(就地说话)」还是「下凡地面演员」;进度可视化 = **太阳本体**由「烧焦的鸡蛋」随 restore 分档复原,**删顶部词 emoji 点灯条**;布景(山壁/河流/房子/云)随太阳复原**灰暗→回春上彩**;月/星在失光与夜间显。

### 15.1 布景构图(千字谷 ch1,全窗竖分)

```
  ✨ ★  ☁️   ☁️       天顶:云(常飘)/月·星(失光或夜间显)
     ☀️ / 🍳烧焦蛋      天幕主位:太阳星体(进度尺,§15.2)
  ⛰️        ⛰️         远景山壁层(灰→彩)
  ⛰️ 🏠 ⛰️ 🏘️ ⛰️      村庄层(山脚;可加 🌳)
   ~~~~~~~🏞️~~~~~~~    近景河流横贯(暗→亮;💧/🌊 波纹)
           🦊          地面演员行(StageCast:灵灵等)
...
- 层间绝对定位叠于 StageFrame;随窗伸缩不滚屏,窄/宽窗不溢出。
- 全由 emoji + CSS 绘制;**位图接缝**:元素/形态视觉集中 `stage-visuals.ts` + 样式 token,换位图只改映射表。

### 15.2 太阳星体状态机(进度尺 · 取代词点灯条)

- **视觉状态**(由恢复进度驱动):`burnt 烧焦蛋(焦黑压暗)` → `crack 裂纹透光(焦壳裂·缝漏光)` → `glow 微亮(转暖黄发光)` → `full 全亮 ☀️(带光晕)`。
- **驱动源** = 本次 ch1 `restored` 层数(共 10:5 词 × sound/shape)。档位阈值与「词里程碑 ↔ 叙事」(词 1 太阳双技能→脱焦、t3「亮」→强光等)在 `ch1` 定稿时对齐,spec 不锁死。
- **契约**:`stage-meta` 纯函数 `sunStage(restored, total) → SunStage`(可单测);布景「回春」彩度 = `worldLight(progress, atmosphere) → CSS`(灰白失饱→撤灰上色,插值/滤镜 plan 定)。
- **月/星**:失光(烧焦蛋未愈)或 `night`/`dark` 幕(break/social/boss)显;`day`/太阳复原后隐。真夜幕太阳**不现身**只月星;**开局(dawn 失光)烧焦蛋 + 月星同框**(太阳光线很暗才见星月,叙事钩子)。

### 15.3 数据扩展(chapter.ts · 可选 · 引擎零读)

```ts
// StageMeta 增可选字段(缺省 = 全员地面,旧数据不崩):
//   sky?: readonly SpeechRole[]   // 当幕作「天空体」的角色:不入 StageCast 地面行,
//                                 // 其台词就地由天空位出泡(如 open 的太阳)。
```

- 演算放 `stage-meta`(cast 拆 ground/sky;天空体须在展示组台词或 cast 以知它说话);`DialoguePresenter`/`StageCast` 消费。

### 15.4 ch1 幕 × 天空(目标态)

| 段 | 天幕太阳 | 月/星 | 布景彩度 | 地面 cast | stage 要点 |
|---|---|---|---|---|---|
| open | 🍳 烧焦蛋(暗·就地说) | 显(失光) | 灰 | 灵灵 | dawn + `sky:[sun]` |
| t1/t2 | 脱焦→裂纹(词 1 后) | 渐退 | 渐回 | 灵灵(+临场) | 叙事对齐阈值 |
| br1~br3 | 隐(真夜) | 显 | 夜 | — | night |
| social-moon | 隐 | 显(月主角) | 夜 | 灵灵+月亮(地面哭诉) | moon 地面 |
| boss | 隐(dark) | 显弱 | 暗 | 灵灵+静默(大) | jingmo 1.5× |
| ending | ☀️ 全亮 | 隐 | 全彩 | 灵灵(+villager) | day;白闪可接 |
| settle | ☀️(结算卡叠彩天) | 隐 | 全彩 | — | day |

### 15.5 文件影响(叠加在 §11)

- `stage.tsx`(改):StageSky → 实景布景层(云/山/村/河/太阳位/月星)+ 世界彩度;StageCast 支持 sky 分流。
- `stage-meta.ts`(改):`sunStage`/`skySplit`/`worldLight` 纯逻辑(+单测)。
- `stage-visuals.ts`(改):太阳 4 形态、布景元素、月星显隐样式。
- `DialoguePresenter.tsx`(改):气泡可锚天空体(天空位弹出)。
- `chapter.ts`(改):`StageMeta.sky?`(§15.3)。
- `ch1.ts`(改):幕 stage 数据补编(§15.4,阈值与叙事对齐)。
- `ChapterRunnerView.tsx`(改):词点灯条退役 → 喂 progress 给布景;`renderDialogue`/`renderStage` 同步。
- `src/index.css`(改):布景/太阳形态/月星/回春 token。
- 测试(改/增):stage-meta 纯逻辑新测;组件断言「词点灯条」→「太阳形态」。

无 engine/chapter-progress/worker/DB/speech 改动。

### 15.6 与既有批次的关系

- §7.2 原把「逐元素精确编排(村庄/月亮/天空各自现身)、ending 白闪、接线 celebrate」留待 PLAN「场景两态升级」行;本批消费其中**天空/布景实景化 + 太阳形态 + 世界回春**核心子集。
- ending 白闪 / 接线 `celebrate` / 更细逐元素编排仍留待该行后续(§10)。
- **验收次序建议**:§15 与 scenes 批同用一份浏览器人工走查(§13 清单按新视觉更新:「词点灯条」断言改「太阳烧焦蛋→复原」),T6 走查推后到本批落地后一并做,避免对将被替换的旧视觉走查两遍。
