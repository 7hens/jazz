...
describe('StageCast', () => {
  it('按 cast 站队出名字牌;说者高亮 accent,听者灰', () => {
    const { container } = render(
      <StageCast cast={['lingling', 'xuwannian']} speaker="xuwannian" />,
    )
    const names = Array.from(container.querySelectorAll('span')).map((n) => n.textContent)
    expect(names).toContain('苏灵灵')
    expect(names).toContain('徐万年')
    // xuwannian 说者高亮 class(accent),苏灵灵听者灰
    const badges = container.querySelectorAll('span')
    const speakerTag = Array.from(badges).find((b) => b.textContent === '徐万年')!
    const listenerTag = Array.from(badges).find((b) => b.textContent === '苏灵灵')!
  ...
  it('bubble 仅渲染在当前说话者上方', () => {
    const { container } = render(
      <StageCast cast={['lingling', 'xuwannian']} speaker="xuwannian" bubble={<span>救救我</span>} />,
    )
  ...
  it('sky 分流:天空体绝不占地面行;说者(天空体)泡/名牌锚天幕,静默天空体零多余 DOM', () => {
    ...
    // 地面行只留 ground(苏灵灵);天空体(徐万年/皮小闹)不落地
    ...
    expect(groundNames).toContain('苏灵灵')
    expect(groundNames).not.toContain('徐万年')
    ...
    expect(container.querySelectorAll('[data-stage-bubble]').length).toBe(1)
    expect(container.textContent).not.toContain('皮小闹')
  })
})
