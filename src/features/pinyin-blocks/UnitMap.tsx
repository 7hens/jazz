    ...
    <div data-unit-map className="min-h-screen px-4 pb-10 pt-4">
      ...
      {/* 成就徽章栏:每格 = 目录里的一条,**按目录全量渲染** —— spec 的「还有几个空着」才是收集动机,
          ...
          **未得态不压 opacity**:整块压透明度会把那圈描边连同底色一起合进天空 —— 徽章栏自己没有底色,
          ...
          亮色主题 ≈3.3:1(#5a7ba0 压天空 #bfe3ff;压渐变末端 #eaf6ff 为 4.0:1);
          暗色主题 ≈8.3:1(#9fb8d4 压 #0b1b3a;压 #12294d 为 7.1:1)。
          ⇒ 同一个 token 在两个主题差 **2.5 倍**,不带主题的那句只对亮色成立。
          已得那格另压在白圆上,圈对 surface:亮 4.4:1(#ffffff)/ 暗 7.8:1(#14223d)。
          **前提:这排仍直落天空** —— 徽章栏自身与祖先
          都没有底色(`App.tsx` 的 `MotionConfig` 不套包裹 div);哪天给它包一层 `bg-surface` 的卡片,
          分母就换成 surface,上面那几个数要重算。数由 token 值合成推得、**非像素裁定**;
