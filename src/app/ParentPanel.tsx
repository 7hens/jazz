                ...
                return (
                  ...
                  >
                    {/* 两态**承重**的只有两处:图标浓淡(未得更淡)与名字后的「· 已得」(只出现在已得那行)。
                        图标浓淡 = 下面那格的 `opacity-45`,这个取值**未经像素裁定** —— 没有任何自动化守卫
                        能判「45% 够不够看得出来」(jsdom 无 CSS 引擎),它是与其它未得态取值对齐的**类比**,
                        别当成已验证;「是否看得出」只有 W-P5 的人眼判据兜。
                        描边与底色都不承重 —— `border-hairline` 对 surface 的合成:
                        亮色主题 rgb(31 58 95 / .10) 压 #ffffff = 1.19:1,
                        暗色主题 rgb(255 255 255 / .14) 压 #14223d = 1.54:1,
                        两个主题都远低于非文本 3:1 ⇒ 虚线实线的深浅看不出来,只当陪衬、别拿它当判据(W-P5 同口径);
                        已得行**不加底色**:`bg-surface-2` 对 surface 亮色只有 1.07:1(#f2f8ff 压 #ffffff,
                        暗色 #1b2e4f 压 #14223d = 1.17:1),同样看不出来,却会把已得那行的说明
                        从 4.40:1 拉到 4.11:1 —— 等于让已得态变成两态里更难读的那个,
                        与本页「还差什么读得清」的取向正相反。
                        不给正文降透明度:说明那行是「还差什么」的出口,整块压暗后 ink-3 对 surface
                        只剩亮色 1.52:1(#8fb0d0 @55% 压 #ffffff)/ 暗色 2.09:1(#5e7fa6 @55% 压 #14223d),
                        两个主题都读不出东西。
