...
- `src/shared/` — 无上层依赖的契约、纯逻辑与中性基础件:`services/*`(契约 + **数据类随契约归属**,`core.ts` 服务访问机制核心:`ServiceToken`·`registry`·`useService`·`useServiceSnapshot` + 快照态 `LoadState`;`api-error`/`load-state`/`types` 已并入 services 后删除)、`utils.ts`(`cn`)、`ui/`(button/card/input/label/badge/select/chart-tooltip,`cva` + `tailwind-merge` 中性视觉基础件,features 可引)。领域数据/规则按语义属主落 feature,不在 shared root 堆(见下 features 分区)
