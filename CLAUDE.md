...
- `src/shared/` — 无上层依赖的契约、纯逻辑与中性基础件:`types.ts`、`words.ts`(100 词词库 + `CATEGORY_LABELS`/`WORDS`/`wordById`)、`progress-rules.ts`(`SKILL_ORDER`/`enabledSkills`/`fullComplete`/`firstTargetId`/`titleForStars`,lesson 与主页共用)、`services/*`(契约 + `core.ts` 服务访问机制核心:`ServiceToken`·`registry`·`useService`·`useServiceSnapshot`)、`utils.ts`(`cn`)、`api-error.ts`/`load-state.ts`、`ui/`(button/card/input/label/badge/select/chart-tooltip,`cva` + `tailwind-merge` 中性视觉基础件,features 可引)
