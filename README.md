...
> **当前状态**:词库学习岛一期(玩法内核 + 趣味系统)已开发完成,前端经架构重构(3 层 feature-slice)后全量单测 / lint / build 通过,待人工浏览器验收与部署。
...
- **运行时出题引擎** —— [src/features/question-engine/engine.ts](src/features/question-engine/engine.ts):每技能步 2 题串行,题型按技能随机组合 —— 拼音 choice / listen-choice、汉字 choice / match、英语 choice / listen-choice / match(填空题型未做,见二期)。干扰项同分类优先、不足跨类兜底、排除与目标词任一文本重复者;选项数按词 id(≤20 给 3 项,其余 4 项);选项 / 题干带全局唯一 id,供 React 复用 key。引擎可注入 `rng` 保证测试确定性。
...
| 项 | 现状 / 处理 |
| --- | --- |
| 浏览器端人工验收 | 尚未在真浏览器点验。走 5 步:登录 → 词 1 三技能学习(含故意答错)→ 结算 → 解锁词 2 → 关拼音后词 2 只 2 步 → 刷新持久 → 重置归零 |
...
| settings 中途开启模块的语义缺口 | 家长学习时关技能、之后重开 → 已学词按新配置重达「全完成」,会再触发 +20 且目标词回跳(重锁已学词)。默认恒定流程下正确;二期以 per-word 永久 `bonus_granted` 列 + ever-enabled 解锁修复 |
| `lesson/mergeProgress` 死导出 | [src/features/lesson/progress.ts](src/features/lesson/progress.ts) 导出未被接线(进度服务自带字段级合并);接入或删除(连同测试) |
...
- **绘画模块**:一词一画 — 拍照 → 缩略 → 亮度抠图 → 自动裁剪 → 确认;`progress.drawing_*` 列后续迁移加入(一期视觉来源 = `word.emoji`)。
- **填空题型**:fillBlank — 汉字补缺字 / 英语补字母(引擎候选已预列,编码未做)。
...
语音识别、汉字书写笔顺、绘画作品检测、社交 / 多人、多孩子档案、家长看板、商店 / 徽章 / 宠物 / 每日挑战、PWA、用户 ID 展示。
