...
- 全量 `npm test`(vitest,jsdom) 全绿,覆盖关卡数据完整性、积木规则(槽位 / 干扰块 / 星级)、结算与成就、各 feature 服务与组件、**worker 路由与 handler 契约**、`architecture.test.ts` 3 层边界;`tsc -b`、`npm run lint`(oxlint)、`npm run build` 通过。
